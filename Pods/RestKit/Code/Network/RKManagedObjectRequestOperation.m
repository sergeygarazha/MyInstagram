...
@synthesize mappingResult = mappingResult;
...
- (void)performMappingOnResponseWithCompletionBlock:(void(^)(RKMappingResult *mappingResult, NSError *error))completionBlock
{
    ...
    if ([fetchRequests count] && [self canSkipMapping]) {
        ...
        RKMappingResult *mappingResult_ = [[RKMappingResult alloc] initWithDictionary:@{ [NSNull null]: managedObjects }];
        completionBlock(mappingResult_, nil);
        return;
    }
    ...
    [self.responseMapperOperation setDidFinishMappingBlock:^(RKMappingResult *mappingResult_, NSError *responseMappingError) {
        if ([weakSelf isCancelled]) return completionBlock(mappingResult_, responseMappingError);
        ...
        success = [weakSelf deleteLocalObjectsMissingFromMappingResult:mappingResult_ error:&error];
        if (! success || [weakSelf isCancelled]) {
            return completionBlock(nil, error);
        }
        ...
        if (mappingResult_) {
            RKRefetchingMappingResult *refetchingMappingResult = [[RKRefetchingMappingResult alloc] initWithMappingResult:mappingResult_
...
- (BOOL)deleteLocalObjectsMissingFromMappingResult:(RKMappingResult *)mappingResult_ error:(NSError **)error
{
    ...
    NSSet *managedObjectsInMappingResult = RKManagedObjectsFromMappingResultWithMappingInfo(mappingResult_, self.mappingInfo) ?: [NSSet set];
